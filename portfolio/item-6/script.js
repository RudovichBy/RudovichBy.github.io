...
var calc = function calc(size, material, options, promocode, result) {
  ...
  var calcFunc = function calcFunc() {
    ...
    if (sizeBlock.value == '' || materialBlock.value == '') {
      resultBlock.textContent = 'Пожалуйсто, выберите размер и материал картины';
...
var forms = function forms() {
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      ...
      item.parentNode.appendChild(statusMessage); //анимация скрытия формы, должна быть подключена animate.css
...
var modals = function modals() {
    ...
    trigger.forEach(function (item) {
      item.addEventListener('click', function (e) {
        ...
        windows.forEach(function (item) {
          item.style.display = 'none'; //добавляем анимацию для popup(должна быть подключена библиотека animate.css)
  ...
  } //вызов popup за определенный промежуток времени
  ...
  function showModalByTime(selector, time) {
    setTimeout(function () {
      ...
      if (!display) {
        ...
        document.body.style.overflow = "hidden"; // убираем сдвиг странице при появлении popup
  ...
  function calcScroll() {
    ...
    div.remove();
    return scrollWidth;
  } // вызываем poopup если далистали страницу до канца и не кликнули не по одной кнопке
...
//подгрузка картинок по клику на кнопку
// вариан №1
...
var sliders = function sliders(slides, dir, prev, next) {
  var slideIndex = 1; // первый слайд который будет отоброжаться 

  var paused = false; // для отключения авто перелистования слайдов при наведени мыши
  ...
  try {
    ...
    prevBtn.addEventListener('click', function () {
      plusSlides(-1); //добавление анимации
  ...
  activateAnimation(); //при наведении мыши отключаем автопереключение слайдов
